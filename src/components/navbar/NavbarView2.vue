  ...
  <nav :class="`${ isScrolled ? 'bg-opacity-100 bg-white duration-200' : 'bg-white duration-200' }  sticky top-0 py-4 px-2 z-20 w-full shadow-md`">
    <transition name="slide">
            <ul v-if="isMenuOpen" class="md:hidden fixed top-0 left-0 mr-10 z-10 w-[250px] sm:w-[400px] h-[100vh]   bg-white shadow-lg pt-5">
              ...
              <li class="ml-2">
                <a href="/tiket" class="hidden md:flex items-center justify-between gap-2 bg-white text-main border border-main py-2 px-4 rounded-lg font-[500] hover:opacity-[0.8] hover:bg-main hover:text-white duration-200" @mouseover="isHovered = true" @mouseleave="isHovered = false"><IcPay :color="isHovered ? '#286895' : 'white'" className="w-[24px]"/> Beli Tiket</a>
              </li>
            </ul>
          </transition>
